        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('google_maps_link')
                    ->label('Lokasi')
                    ->formatStateUsing(fn ($state) => $state ? '📍 Lihat Lokasi' : '—')
                    ->url(fn ($record) => $record->google_maps_link, shouldOpenInNewTab: true)
                    ->color(fn ($record) => $record->google_maps_link ? 'primary' : 'gray'),
            ])
            ->filters([])
            ->recordActions([
